// العنوان الحالي يُستخدم للخادم المحلي (Localhost)
// إذا كان خادم الـ API غير مُشغّل، أو كانت هناك مشكلة في شهادة SSL/HTTPS، أو مشكلة CORS (Cross-Origin Resource Sharing)، فسيتم منع الريكويست.
// يرجى التأكد من تكوين CORS في الخادم الخلفي للسماح بالاتصال من المصدر الذي يتم تشغيل الصفحة منه.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * تملأ قائمة البيانات (datalist) بأسماء المحطات العربية لإكمال الإدخال تلقائيًا.
     */
    function populateDatalist() {
        stationsDatalist.innerHTML = '';
        allStations.forEach(station => {
            const option = document.createElement('option');
            // استخدام الاسم العربي كقيمة لكي يظهر للمستخدم في قائمة الإكمال التلقائي
    ...
    /**
     * تبحث عن مُعرف المحطة (StationID) بناءً على الاسم المُدخل وتُحدّث الحقل المخفي.
     * @param {HTMLInputElement} inputElement - حقل إدخال اسم المحطة (من أو إلى).
     * @param {HTMLInputElement} hiddenIdElement - حقل الإدخال المخفي لمُعرّف المحطة.
     */
    function updateHiddenId(inputElement, hiddenIdElement) {
        const stationName = inputElement.value.trim();
        // البحث عن المحطة المطابقة باستخدام الاسم العربي
        const selectedStation = allStations.find(s => s.StationNameAR === stationName);

        if (selectedStation) {
            // تعيين القيمة إذا تم العثور على تطابق
            hiddenIdElement.value = selectedStation.StationID;
            // يمكن أيضًا مسح أي رسائل خطأ سابقة
            searchMessage.textContent = '';
        } else {
            // مسح القيمة إذا لم يتم العثور على تطابق
            hiddenIdElement.value = '';
            // لا نعرض رسالة خطأ هنا، نعتمد على التحقق في دالة الإرسال.
        }
    }
    ...
    // ربط مستمعي حدث 'change' لتحديث الحقول المخفية فور تغيير المحطات
     ...
     * يقوم بالتحقق من المدخلات، وتحديث المعرّفات المخفية، ثم إرسال طلب البحث إلى الـ API.
     */
    searchForm.addEventListener('submit', async (e) => {
        // منع السلوك الافتراضي لإرسال النموذج (الذي يؤدي إلى إعادة تحميل الصفحة)
        e.preventDefault(); 
        
        // تحديث المعرفات المخفية مرة أخيرة للتأكد من التقاط القيمة الحالية عند الضغط على زر الإرسال
        updateHiddenId(fromInput, fromIdHidden);
        updateHiddenId(toInput, toIdHidden);
        ...
        // التحقق من أن أسماء المحطات غير فارغة
        if (!departureStationName || !arrivalStationName) {
            searchMessage.textContent = "يرجى اختيار محطة انطلاق ووصول صحيحة من القائمة.";
            searchMessage.style.color = '#dc3545'; // لون أحمر للخطأ
            return;
        }

        searchMessage.textContent = "جاري البحث عن الرحلات...";
        searchMessage.style.color = '#ffc107'; // لون أصفر/ذهبي للانتظار
        
        // استخدام أسماء المحطات في البحث
        const fullUrl = `${BASE_API_URL}?departureStationName=${encodeURIComponent(departureStationName)}&arrivalStationName=${encodeURIComponent(arrivalStationName)}`;
        
        try {
            // محاولة إرسال الريكويست
            const response = await fetch(fullUrl, {
                method: 'GET',
                // لا نحتاج لـ Headers خاصة بـ Content-Type لطلب GET بدون جسم (body)
            });
            
            // محاولة تحليل استجابة JSON، وتجنب الخطأ إذا كانت الاستجابة فارغة
            const data = await response.json().catch(() => ({}));
            
            if (response.ok) {
                // نجاح الاستجابة (كود 200-299)
                searchMessage.textContent = `تم العثور على ${data.length || 0} رحلة. جاري الانتقال...`;
                searchMessage.style.color = '#28a745'; // لون أخضر للنجاح
                
                // حفظ نتائج البحث وأسماء المحطات في الـ localStorage
                ...
                // فشل الاستجابة (كود 400، 500، إلخ)
                const errorMessage = data.message || data.title || "فشل البحث. يرجى التحقق من المدخلات.";
                searchMessage.textContent = `خطأ في الخادم (HTTP ${response.status}): ${errorMessage}`;
                searchMessage.style.color = '#dc3545';
                console.error('API Error:', response.status, data);
            }

        } catch (err) {
            // خطأ في الشبكة (Network Error) مثل مشكلة CORS أو خادم غير مُشغّل
            searchMessage.textContent = "لا يمكن الاتصال بالخادم الآن. يرجى التأكد من تشغيل الـ API وحل مشكلة CORS/SSL إذا كانت موجودة.";
            searchMessage.style.color = '#dc3545';
            console.error('Network/Fetch Error (CORS/SSL?):', err);
        }
    });
});